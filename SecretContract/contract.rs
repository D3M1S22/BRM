use crate::{
    msg::{
        ExecuteMsg, GatewayMsg, InstantiateMsg, MetadataStoreMsg, QueryMsg,
        ResponseMetadataRetrieveMsg, ResponseMsg, ResponsePrivateMetadataRetrieveMsg,
    },
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    let response = match msg {
        ...
        QueryMsg::RetrievePrivateMetadata { token_id, proof } => {
            retrieve_private_metadata(deps, token_id, proof)
        }
    };
...
fn execute_store_confidential_metadata(
    ...
    // Deserialize input into MetadataStoreMsg (assuming this now reflects the updated structure)
    ...
    // The private_metadata should now be a PrivateMetadata struct
    let private_metadata = input.private_metadata;

    let confidential_metadata = ConfidentialMetadata {
        owner,
        token_id,
        private_metadata, // Now of type PrivateMetadata
    };

    // Store the confidential metadata in the CONFIDENTIAL_METADATA map
    ...
    // Create the callback message with the result, task, and input hash
...
fn retrieve_private_metadata(deps: Deps, token_id: u64, proof: String) -> StdResult<Binary> {
    let value = CONFIDENTIAL_METADATA
        .get(deps.storage, &token_id)
        .ok_or_else(|| StdError::generic_err("Value not found"))?;

    if proof != "secret" {
        return Err(StdError::generic_err("Invalid proof".to_string()));
    }

    to_binary(&ResponsePrivateMetadataRetrieveMsg {
        private_metadata: value.private_metadata, // Access the field after unwrapping
    })
}
